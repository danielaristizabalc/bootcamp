...
public class BootcampUseCase implements BootcampServicePort {
    ...
    public Mono<Bootcamp> registerBootcamp(Bootcamp bootcamp, String messageId) {
        return validateCapabilitiesCount(bootcamp)
            ...
            .doOnSuccess(savedBootcamp -> publishBootcampReportInBackground(savedBootcamp, messageId));
    }
    ...
    private void publishBootcampReportInBackground(Bootcamp savedBootcamp, String messageId) {
        if (savedBootcamp == null) {
            return;
        }

        capabilityGateway.findCapabilitiesByIds(savedBootcamp.capabilityIds(), messageId)
                .flatMap(capabilities -> reportGateway.sendBootcampCreatedReport(savedBootcamp, capabilities, messageId))
                .subscribeOn(Schedulers.boundedElastic())
                .doOnError(error -> log.error("Error sending bootcamp report for messageId: {}", messageId, error))
